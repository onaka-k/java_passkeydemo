...
## H2 ConsoleでDB確認
1. アプリ起動中に `http://localhost:8080/h2-console` を開く
2. JDBC URL は `jdbc:h2:file:./data/passkeydemo;AUTO_SERVER=TRUE` を指定
3. User Name は `sa`、Password は空欄で接続

※ `app_user` / `passkey_credential` テーブルの中身を画面から確認できます。
